...
class ParticleHongPencilBeamEngine(ParticlePencilBeamEngineAbstract):
    ...
    def _calc_particle_bixel_gpu(self, bixel,radial_dist_sq):
            kernels = self._interpolate_kernels_in_depth(bixel)
            ...
            if self.lateral_model == "single":
                ...
                lateral=cp.empty(s,dtype=cp.float32)
                sigma_1=cp.asarray(kernels["sigma_1"])
                sigma_2=cp.asarray(kernels["sigma_2"])
                weight=cp.asarray(kernels["weight"])

                self.calc_particle_bixel_double_gpu[(s+128-1)//128,128](sigma_1,sigma_2,bixel["sigma_ini_sq"],radial_dist_sq,weight,lateral)
            elif self.lateral_model == "multi":
                sigma_sq = kernels["sigma_multi"] ** 2 + bixel["sigma_ini_sq"]
                lateral = xp.sum(
                    (
            ...
            bixel["physical_dose"] = pb_kernel.lateral_cut_off.comp_fac * lateral.get() * kernels["idd"]
